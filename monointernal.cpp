...
{
	return mono_string_new(mono_domain_get(), QCoreApplication::applicationName().toStdString().c_str());
}
...
GlueSvgWidget* qt_svgwidget_new(MonoObject* obj, QWidget* parent)
{
	return new GlueSvgWidget(obj, parent);
}

void qt_svgwidget_delete(GlueSvgWidget* widget)
{
	delete widget;
}
void qt_svgwidget_load(QSvgWidget* widget, MonoArray* data)
{
	char* _data = (char*)mono_array_addr(data, uint8_t, 0);
	QByteArray svgData(_data);
	widget->load(svgData);
}
QSize* qt_svgwidget_sizehint_get(QSvgWidget* widget)
{
...
{
	widget->updateProgressValue(value);
}
...
{
	widget->updateMaximumValue(value);
}
...
int qt_keyevent_count_get(QKeyEvent* event)
{
	return event->count();
}
...
int qt_tableview_move_cursor(GlueTableView* tableView, int cursorAction, int modifier)
{
...
QPainter* qt_painter_new(QPixmap* pixmap)
{
...
#include <QtCore/5.15.13/QtCore/private/qeventdispatcher_glib_p.h>
#include <QtCore/5.15.13/QtCore/private/qeventdispatcher_unix_p.h>
...
extern "C" void qt_application_monointernal_init()
{
	...
	mono_add_internal_call("Qt.SvgWidget::qt_svgwidget_new", reinterpret_cast<void*>(qt_svgwidget_new));
